...
config_DESeq <- 0
for (x in list.files()) {
  if (grepl("Config", x, fixed = TRUE) & grepl("D2.", x, fixed = TRUE)) {
    config_DESeq <- x
    print(config_DESeq)
    ase <- FALSE
    ...
    for (file in files) {
      ...
      geneInfo <- subset(geneInfo, geneInfo$gene %in% keep)
      joined <- inner_join(f, geneInfo, b = c("V1" = "gene"))
      rownames(joined) = joined$V1
      joined <- subset(joined, joined$V1 %in% keep)
      keep <- joined$V1
      joined <- dplyr::select(joined,-V1)
      f_join <- data.frame(joined$V2)
      row.names(f_join) <- row.names(joined)
      info_join <- dplyr::select(joined,-V2)
      
      info_join$genesizeKb = as.vector(as.numeric(info_join$merged)/1000)
      f_tot_cpm = sweep(as.matrix(f_join), 2, as.double(colSums(f_join)/1000000), `/`)
      f_tot_fpkm = sweep(as.matrix(f_tot_cpm), 1, as.double(info_join$genesizeKb), `/`)
      f_tot_rpk = sweep(as.matrix(f_join), 1, as.double(info_join$genesizeKb), `/`)
      f_tot_tpm = sweep(as.matrix(f_tot_rpk), 2, as.double(colSums(f_tot_rpk)/1000000), `/`)
      t <- c(t, file)
      if (z == 0) {
        a <- as.data.frame(f_tot_tpm)
        colnames(a) <- t
        z <- 1
      } else {
        colnames(f_tot_tpm) <- c(file)
        a <- merge(a, f_tot_tpm, by = 'row.names', copy = TRUE)
        rownames(a) <- a$Row.names
        a <- dplyr::select(a, -Row.names)
      }
    }
    #rownames(a) <- rownames(f_tot_tpm)
    #colnames(a) <- t
