...
rule count_ase:
	input:
		...
		reads='analysis/Mapped/{species}/{sample}/rmdup.sort.bam',
		readsidx='analysis/Mapped/{species}/{sample}/rmdup.sort.bam.bai' #just so the run wouldn't start when this file is not ready yet. It is needed for the script, but the input is internal, no need to specify it
	output:
